...
echo "<title>".$FormDiscountTitle3."</title>";
						...
						//Таблиця 
							echo "<TR>";
								echo "<TD align='left'>";
									...
									{
										$AptekaList[$i]->Connect();;
									}
								
									for ($i=1; $i<=Count($AptekaList); $i++)
									{
										if ($AptekaList[$i]->Active==1)
										{
											$DataModule = new DataModule($AptekaList[$i]);
											$ID = $DataModule->LinkCardToAnalitic($_REQUEST["card"], $_REQUEST["analitic"]);
											echo $AptekaList[$i]->FullName." - ";
											if ($ID>0)
											{
												echo "OK<BR>";
											}
